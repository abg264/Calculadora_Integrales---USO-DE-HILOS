...
class MathEngine:
    ...
    def default_variables_for_type(self, expr, idx_type):
        # El índice 3 ahora es la Integral Doble Polar
        defaults = { 0: ["x"], 1: ["y", "x"], 2: ["z", "y", "x"], 3: ["r", "theta"] }
    ...
    def variables_from_differentials(self, diffs, idx_type):
        ...
        for diff in diffs:
            if diff == "A":
                # Asignar a Doble Polar
                variables.extend(["r", "theta"] if idx_type == 3 else ["y", "x"])
            elif diff == "V":
    ...
    def process_integral(self, expr_str, idx_type, is_indefinite, limit_strings, lang="es"):
        ...
        try:
            ...
            # Mapeo actualizado: 0: Sencilla (1), 1: Doble (2), 2: Triple (3), 3: Doble Polar (2)
            expected = {0: 1, 1: 2, 2: 3, 3: 2}.get(idx_type, 1)
            if not is_indefinite and len(variables) > expected:
                raise ValueError("Hay mas diferenciales que limites disponibles para el tipo de integral seleccionado." if lang == "es" else "There are more differentials than limits available for the selected integral type.")
            ...
            # Conversión Automática a Polares (Índice 3 para Integral Doble Polar)
            if idx_type == 3:
            ...
            if idx_type in (0, 1, 2):
                ...
                # Verificar si el usuario pidió explícitamente integrar en r o theta
            ...
            # Aplicar Jacobiano Polar en Integral Doble Polar (Índice 3)
            if idx_type == 3:
                # CONDICIÓN ESTRICTA: SOLO se agregará si el usuario escribió explícitamente 'dA'
